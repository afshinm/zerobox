...
/// Returns the path to the MITM CA certificate if it exists.
/// The cert is generated by the proxy and used to sign intercepted TLS connections.
/// Respects CODEX_HOME env var, defaults to ~/.zerobox.
pub fn mitm_ca_cert_path() -> Option<PathBuf> {
    let base = std::env::var("CODEX_HOME")
        .map(PathBuf::from)
        .ok()
        .or_else(|| dirs::home_dir().map(|h| h.join(".zerobox")))?;
    let path = base.join("proxy").join("ca.pem");
