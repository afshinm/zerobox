...
pub enum FileSystemSpecialPath {
    ...
    Tmpdir,
    SlashTmp,
    /// Preserves unknown special-path tokens so older runtimes can ignore
    /// them without rejecting config authored by a newer release.
    Unknown {
        path: String,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        #[ts(optional)]
...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Protect .codex directory as read-only when it exists.
    #[allow(clippy::expect_used)]
